...
class ArithmeticConstraint
  ...
  def new_value(connector)
    ...
    # -------------------- ADDED --------------------
    elsif [b,out].include?(connector) and (not a.has_value?) and b.has_value? and 
         out.has_value? then
      val=out.value.send(inverse_op, b.value)
      logger.debug("#{self} : #{out} updated")
      a.assign(val, self)
      
    elsif [a,out].include?(connector) and a.has_value? and (not b.has_value?) and 
         out.has_value? then
...
def celsius2fahrenheit 
  ...
  nine_c = Connector.new("9c")
  f_thirtytwo = Connector.new("f-32")
  
  nine = Connector.new("9", 9)
  five = Connector.new("5", 5)
  thirtytwo = Connector.new("32", 32)

  Multiplier.new(c, nine, nine_c)
  Multiplier.new(f_thirtytwo, five, nine_c)
  Adder.new(f_thirtytwo, thirtytwo, f)
  return c, f
end
