...
class ConstraintParser < Parser
  ...
  def get_connector(x)
    if x.is_a?(Connector) then
      x
    elsif x.is_a?(ArithmeticConstraint) then
      # Eftersom x -> (32 + f-32 == f) vill vi åt b, inte out
      # dock stämmer det inte för andra, så felet kan ligga här?
      # men det blir ännu mer fel om vi använder x.out, då försvinner f-32 helt 
      x.b
    end
  end
